...
class GviaDailyPerMonth(object):
    def __init__(self, status, requested_year, requested_month):
        ...
        FROM
          ...
          LEFT JOIN (
                      ...
                      WHERE YEAR(DatePay) = {requested_year} AND MONTH(DatePay) = {requested_month}
                    ) taxesPay
        ...
        ORDER BY [צוות]'''.format(requested_year=requested_year, requested_month=requested_month)
        ...
        month_report_file_name = u'תכנון הצפי לחודש {month}-{year}.xlsx'.format(month=self.month, year=self.year)
        self.df_from_month_report = pd.read_excel(month_report_file_name, convert_float=True, sheetname=u'דוח גביה חודשי- לקוחות פעילים')
    ...
    def get_df_of_fees(self):
        ...
        for index, row in df.iterrows():
            ...
            if row['DatePay'] is not None and row['DatePay'].year == self.year and row['DatePay'].month == self.month:
                date = row['DatePay']
                dates.append(date)
            else:
    ...
    def create_report_for_each_team(self):
        gvia_for_teams = GviaDailyPerMonth('report for teams', self.year, self.month)
        rows = len(self.df.index)
        ...
        for r in range(0, rows):
            if r == rows - 1:
                ...
                file_name = u' דוח גביה יומי {month}-{year} צוות {team}.xlsx'.format(team=team, month=self.month,
                                                                                     year=self.year)
    ...
    def change_types_of_cells(self):
        ...
        wb = load_workbook(u' דוח גביה יומי {month}-{year}.xlsx'.format( month=self.month, year=self.year))
        ws = wb.active
        ...
        wb.save(u' דוח גביה יומי {month}-{year}.xlsx'.format(month=self.month, year=self.year))

    def run_report(self):
        self.create_df()
        self.create_separated_df(u' דוח גביה יומי {month}-{year}.xlsx'.format(month=self.month,
                                                                                    year=self.year),
                                       'report_for_hani')
...
if __name__ == '__main__':
    gvia_daily = GviaDailyPerMonth('report_for_hani', 2016, 8)
    gvia_daily.create_df()
    gvia_daily.create_separated_df(u' דוח גביה יומי {month}-{year}.xlsx'.format(month=gvia_daily.month,
                                                                                year=gvia_daily.year), 'report_for_hani')
